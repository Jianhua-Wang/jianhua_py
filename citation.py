...
def get_citation(doi):
    ...
    journal_names = pd.Series(
        index=journal_names[1].values, data=journal_names[5].values)
    headers = {
    ...
    given_name = ' '.join(
        [i[0].upper()+'.' for i in re.split(' |-', given_name)])
    print('SHORT CITATION')
    print(f'{family_name}, {given_name} et al. {journal_short} ({year})')
    print('\nFULL CITATION')
    print(
        f'{family_name}, {given_name} et al. {year}. {title}. {journal}. DOI:[{doi}](http://dx.doi.org/{doi})')
